using AppForSEII2526.API;
using AppForSEII2526.API.Controllers;
using AppForSEII2526.API.DTOs.ReviewDTO;
using Microsoft.AspNetCore.Mvc;
using Microsoft.Extensions.Logging;
using System;
using System.Collections.Generic;
using System.Linq;
using System.Threading.Tasks;

namespace AppForSEII2526
{
    // ============================================================
    // TESTS PARA EL MÉTODO GETREVIEW DEL CONTROLADOR ReviewController
    // ============================================================
    // Esta clase hereda de AppForSEII25264SqliteUT para usar una
    // base de datos temporal en memoria en cada ejecución.
    public class GetReview_test : AppForSEII25264SqliteUT
    {
        public GetReview_test()
        {
            // ------------------------------------------------------
            // CREAMOS MODELOS DE COCHES DISPONIBLES
            // ------------------------------------------------------
            var models = new List<Model>()
            {
                new Model("1", "Q5"),
                new Model("2", "Civic"),
            };

            // ------------------------------------------------------
            // CREAMOS COCHES ASOCIADOS A ESOS MODELOS
            // Cada coche tiene fabricante, color, tipo de combustible, etc.
            // ------------------------------------------------------
            var cars = new List<Car>()
            {
                new Car("Turismo","Blanco","","1.8","Gasolina",1,"Estandar","Audi",18000,2,4,180,18,models[0]),
                new Car("Familiar","Azul","","2.0","Hibrido",2,"Estandar","Honda",22000,4,5,160,20,models[1])
            };

            // ------------------------------------------------------
            // CREAMOS UN USUARIO PARA ASOCIAR A LA REVIEW
            // Este usuario será el "autor" de la reseña.
            // ------------------------------------------------------
            var user = new ApplicationUser("Avda. España 1", "600000001", "Manuel", "Garcia", "manolito")
            { Id = "U1" };

            // ------------------------------------------------------
            // CREAMOS LA REVIEW QUE ESTARÁ EN LA BASE DE DATOS
            // Esta es la review que luego recuperaremos con el test
            // ------------------------------------------------------
            var review = new Review(1, new DateTime(2025, 10, 20))
            {
                Country = "España",
                DriverType = "Habitual",
                Client = user,
                Cars = new List<ReviewItem>()
            };

            // ------------------------------------------------------
            // AÑADIMOS LOS ITEMS DE LA REVIEW
            // Cada ReviewItem es una opinión sobre un coche concreto.
            // ------------------------------------------------------
            var reviewItems = new List<ReviewItem>()
            {
                new ReviewItem(cars[0].Id, "Buen coche para ciudad", 5, review.Id),
                new ReviewItem(cars[1].Id, "Cómodo y eficiente", 4, review.Id)
            };

            // Se asignan los items a la review
            review.Cars = reviewItems;

            // ------------------------------------------------------
            // GUARDAMOS TODO EN LA BASE DE DATOS DE PRUEBA
            // ------------------------------------------------------
            _context.AddRange(models);
            _context.AddRange(cars);
            _context.Add(user);
            _context.Add(review);
            _context.AddRange(reviewItems);
            _context.SaveChanges();
        }

        // ============================================================
        // TEST: LA REVIEW EXISTE → SE DEVUELVE CORRECTAMENTE (200 OK)
        // ============================================================
        [Fact]
        [Trait("Database", "WithoutFixture")]
        [Trait("LevelTesting", "Unit Testing")]
        public async Task GetReview_Found_test()
        {
            // Creamos un logger falso y el controlador con el contexto de prueba
            var mock = new Mock<ILogger<ReviewController>>();
            ILogger<ReviewController> logger = mock.Object;
            var controller = new ReviewController(_context, logger);

            // Creamos el DTO que ESPERAMOS recibir de la API
            var expected = new ReviewDetailDTO(
                1,
                "U1",
                new DateTime(2025, 10, 20),
                "Manuel",
                "España",
                "Habitual",
                new List<ReviewItemDTO>
                {
                    new ReviewItemDTO(1, "Q5", "Audi", "Gasolina", "Blanco", "Buen coche para ciudad", 5),
                    new ReviewItemDTO(2, "Civic", "Honda", "Hibrido", "Azul", "Cómodo y eficiente", 4)
                }
            );

            // Llamamos al método real del controlador
            var result = await controller.GetReview(1);

            // Verificamos que responde con 200 OK
            var okResult = Assert.IsType<OkObjectResult>(result);

            // Obtenemos el objeto devuelto
            var reviewDTOActual = Assert.IsType<ReviewDetailDTO>(okResult.Value);

            // Comparamos datos de la review
            Assert.Equal(expected,reviewDTOActual);
            
        }

        // ============================================================
        // TEST: LA REVIEW NO EXISTE → DEVUELVE NotFound (404)
        // ============================================================
        [Fact]
        [Trait("Database", "WithoutFixture")]
        [Trait("LevelTesting", "Unit Testing")]
        public async Task GetReview_NotFound_test()
        {
            var mock = new Mock<ILogger<ReviewController>>();
            ILogger<ReviewController> logger = mock.Object;
            var controller = new ReviewController(_context, logger);

            // Intentamos obtener una review con un ID que es MENOR DE 0
            var result = await controller.GetReview(-1);

            // La API debe devolver un NotFound
            Assert.IsType<NotFoundResult>(result);
        }
    }
}
